...
x <- cbind(x, percent_overall=x$Positive_overall/x$Total_overall*100)

x <- cbind(x, lo=NA, hi=NA)
for(i in 1:nrow(x)) {
    ci <- binom.test(x[i,"Positive_overall"], x[i, "Total_overall"])$conf.int
    x[i,"lo"] <- ci[1]*100
    x[i,"hi"] <- ci[2]*100
}
```
...
mx <- max(x[,"hi"])
par(cex=1.3)
z <- barplot(x[, "percent_overall"], col=light_color, main="",
